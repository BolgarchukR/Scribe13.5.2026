...
class VoskRecognizer(QObject):
    ...
    text_recognized = pyqtSignal(str, bool, str)  # Emitted when text/command is processed (text, is_command, command_name)
    ...
    def _apply_final(self, final_text: str):
        ...
        if self.final_handler:
            ...
            if isinstance(res, tuple) and len(res) >= 2:
                # New format: (action_callable, trigger_name)
                command_action, command_name = res
                handled_as_command = True
            elif callable(res):
                handled_as_command = True
                command_action = res
            elif isinstance(res, str):
                final_text_plain = res
                diff_text = res
            elif res is True:
                handled_as_command = True
        ...
        if handled_as_command:
            logger.debug("[Final->apply] Command executed! Erasing partial_prev chars.")
            if self.partial_prev:
                # Erase text typed during thinking
                self.inserter.erase_chars(len(self.partial_prev))
                if hasattr(self.inserter, 'wait_until_idle'):
                    self.inserter.wait_until_idle()
            ...
            if command_action:
                try:
                    ...
                    logger.error(f"[Final->apply] Error executing command: {e}")
        ...
        logger.debug(f"[✓] {final_text_plain}")
        self.text_recognized.emit(final_text_plain, handled_as_command, command_name)
